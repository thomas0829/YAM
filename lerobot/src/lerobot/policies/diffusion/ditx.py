...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        # Separate projections so we can use different sources for Q vs K/V
    ...
    def forward(self, x: torch.Tensor, kv: torch.Tensor | None = None, attn_mask=None) -> torch.Tensor:
        """
        Args:
            x:  (B, N, C) queries
            kv: (B, M, C) key/value source (if None, uses x -> self-attn)
            attn_mask: optional attention mask broadcastable to (B, num_heads, N, M)
        """
        B, N, C = x.shape
        if kv is None:
            kv = x
...
class DiTXBlock(nn.Module):
    ...
    def forward(self, x, t, c, attn_mask=None):
        ...
        x = x + gate_msa.unsqueeze(1) * self.attn1(modulate(self.norm1(x), shift_msa, scale_msa), attn_mask=attn_mask) # norm, scale&shift, attn, scale,
        x = x + gate_mca.unsqueeze(1) * self.attn2(modulate(self.norm2(x), shift_mca, scale_mca), kv=c, attn_mask=attn_mask) # norm, scale&shift, attn, scale,
...
class DiffusionTransformerX(nn.Module):
    """
    ...
    def __init__(self, config: DiffusionConfig):
        ...
        self.time_embedder = nn.Linear(config.diffusion_step_embed_dim, config.embed_dim)
        # self.cond_embedder = nn.Linear(global_cond_dim, config.embed_dim)
        ...
        self.context_pos_embed = LearnedSinusoidalPosEmb(config.embed_dim)

        self.blocks = nn.ModuleList([
            DiTXBlock(config.embed_dim, config.num_heads, mlp_ratio=config.mlp_ratio) for _ in range(config.depth)
        ])
    ...
    def forward(self, x: torch.Tensor, timesteps_embed: torch.Tensor | int, global_cond=None) -> torch.Tensor:
        ...
        """
        timesteps_feature = timesteps_embed
        timesteps_feature_embed = self.time_embedder(timesteps_feature)
        ...
        state_embed = self.state_embedder(state_embed) # (B, S, D_embed)
        global_feature.append(state_embed)

        if self.config.image_features:
            img_embed = global_cond_temp.pop(0) # (B, S, N * L_img, D_img)
            img_embed = self.image_embedder(img_embed).view(B, -1, self.config.embed_dim) # (B, S * N * L_img, D_embed)
            global_feature.append(img_embed)

        if self.config.env_state_feature:
            env_state_embed = global_cond_temp.pop(0) # (B, S, D_state)
            env_state_embed = self.env_state_embedder(env_state_embed) # (B, S, D_embed)
            global_feature.append(env_state_embed)

        if self.config.use_language:
            lang_features = global_cond_temp.pop(0) # (B, L_lang, D_lang)
            lang_features = self.text_embedder(lang_features) # (B, L_lang, D_embed)
            global_feature.append(lang_features)

        global_feature = torch.cat(global_feature, axis=1) # (B, L_total, D_embed)
        B, T_context, _ = global_feature.shape

        global_feature_embed = global_feature + self.context_pos_embed(torch.arange(T_context, device=x.device)).expand(B, -1, -1)

        B, T_x, _ = x.shape
        x = self.x_embedder(x) + self.x_pos_embed(torch.arange(T_x, device=x.device)).expand(B, -1, -1)

        for block in self.blocks:
            # x = block(x, c, attn_mask=self.mask)  # (N, T, D)
            x = block(x, timesteps_feature_embed, global_feature_embed, attn_mask=None)  # (N, T, D)
        x = self.final_layer(x, timesteps_feature_embed)  # (N, T, output_dim)
        return x
