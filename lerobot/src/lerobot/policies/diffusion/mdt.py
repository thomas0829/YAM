...
class MDTBlock(nn.Module):
    ...
    def forward(self, x, t, c, attn_mask=None):
        ...
        x = x + self.attn2(self.norm2(x), kv=c, attn_mask=attn_mask) # norm
...
class MultimodalDiffusionTransformer(nn.Module):
    ...
    def __init__(self, config: DiffusionConfig):
        ...
        self.blocks = nn.ModuleList([
            MDTBlock(config.embed_dim, config.num_heads, mlp_ratio=config.mlp_ratio) for _ in range(config.depth)
        ])
