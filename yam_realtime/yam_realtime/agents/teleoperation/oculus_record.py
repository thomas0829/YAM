...
class OculusRecordAgent(Agent):
    ...
    def _update_internal_state(self, num_wait_sec = 5, hz=30):
        ...
        while True:
            ...
            # Debug: print button values when grip is pressed
            if buttons and buttons.get("RG", False):
                trig_val = buttons.get('rightTrig', (0.0,))
                if isinstance(trig_val, tuple):
                    trig_val = trig_val[0]
            ...
            # Always handle both left and right, even in single-arm mode
            if "left" not in self.state["movement_enabled"]:
                self.state["movement_enabled"]["left"] = False
            if "right" not in self.state["movement_enabled"]:
                self.state["movement_enabled"]["right"] = False
                
            toggled = {"left": self.state["movement_enabled"]["left"] != buttons.get("LG", False), "right": self.state["movement_enabled"]["right"] != buttons.get("RG", False)}
            self.update_VR = {"left": self.update_VR["left"] or buttons.get("LG", False), "right": self.update_VR["right"] or buttons.get("RG", False)}
            ...
            self.state["movement_enabled"] = {"left": buttons.get("LG", False), "right": buttons.get("RG", False)}
    ...
    def _calc_delta_action(self, viser_state_dict):
        ...
        # update VR state
        # In single-arm mode, use right controller to control left arm
        if self.bimanual:
            ...
            # Single-arm mode: use right controller for left arm
            if self.state["movement_enabled"]["right"]:
                if self.update_VR["right"]:
                    self._process_VR_reading_right()
                    self.update_VR["right"] = False
        ...
        if self.bimanual:
            ...
            viser_pos_right = np.zeros(3)
            viser_quat_right = np.array([0, 0, 0, 1])
            viser_gripper_right = 0.0
        # print(robot_pos, robot_quat)

        # Reset origin if needed
        # In single-arm mode, use right controller for left arm
        if self.bimanual:
            ...
            # Single-arm: right controller controls left arm
            if self.reset_origin["right"] and self.vr_state["right"] is not None:
                self.viser_origin["left"] = {"pos": viser_pos_left, "quat": viser_quat_left, "gripper": viser_gripper_left}
                self.vr_origin["left"] = {"pos": self.vr_state["right"]["pose"], "quat": self.vr_state["right"]["quat"], "gripper": self.vr_state["right"]["gripper"]}
                self.reset_origin["right"] = False
                self.vr_prev_state["right"] = self.vr_state["right"].copy()
            
        if self.bimanual and self.reset_origin["right"] and self.vr_state["right"] is not None:
        ...
        if self.bimanual:
            ...
            # Single-arm: right controller controls left arm
            if self.vr_state["right"] is not None and self.vr_prev_state["right"] is not None:
                delta_pos_left = self.vr_state["right"]["pose"] - self.vr_prev_state["right"]["pose"]
                delta_quat_left = quat_diff(self.vr_state["right"]["quat"], self.vr_prev_state["right"]["quat"])   
                viser_desired_pos_left = viser_pos_left + delta_pos_left
                viser_desired_quat_left = (R.from_quat(delta_quat_left) * R.from_quat(viser_quat_left)).as_quat()
    ...
    def _setup_visualization(self):
        ...
        self.left_gripper_slider_handle = self.viser_server.gui.add_slider(
            "Right Gripper", min=0.0, max=1.0, step=0.01, initial_value=0.0
        )
        ...
        if self.bimanual and self.right_arm_extrinsic is not None:
            ...
            self.right_gripper_slider_handle = self.viser_server.gui.add_slider(
                "Left Gripper", min=0.0, max=1.0, step=0.01, initial_value=0.0
            )
    ...
    def act(self, obs: Dict[str, Any]) -> Any:
        ...
        viser_state = {
            "pos": {"left": np.concatenate([pose_left, xyzw_left])},
            "gripper" : {"left": obs["left"]["gripper_pos"]},
        }
        ...
        if self.bimanual:
            ...
            viser_state["pos"]["right"] = np.concatenate([pose_right, xyzw_right])
            viser_state["gripper"]["right"] = obs["right"]["gripper_pos"]
        ...
        if viser_action is not None:
            ...
            # In single-arm mode, use right controller trigger for left arm gripper
            if self.bimanual:
                self.left_gripper_slider_handle.value = 1 - self.state["buttons"].get("leftTrig", (0.0,))[0]
            else:
                trig_value = self.state["buttons"].get("rightTrig", (0.0,))
                if isinstance(trig_value, tuple):
                    trig_value = trig_value[0]
                gripper_value = 1 - trig_value
                self.left_gripper_slider_handle.value = gripper_value
    ...
    def get_info(self):
        try:
            state_info = {
                "success" : self.state["buttons"].get("A", False),
                "failure" : self.state["buttons"].get("B", False),
                "X" : self.state["buttons"].get("X", False),
                "Y" : self.state["buttons"].get("Y", False),
                "left_gripper" : self.state["buttons"].get("leftTrig", (0.0,))[0] if isinstance(self.state["buttons"].get("leftTrig", (0.0,)), tuple) else self.state["buttons"].get("leftTrig", 0.0),
                "right_gripper" : self.state["buttons"].get("rightTrig", (0.0,))[0] if isinstance(self.state["buttons"].get("rightTrig", (0.0,)), tuple) else self.state["buttons"].get("rightTrig", 0.0),
                "movement_enabled" : self.state["movement_enabled"],
                "controller_on" : self.state["controller_on"],
            }
        except Exception as e:
            raise KeyError(
                f"Error reading VR controller state: {e}"
            )
        return state_info
