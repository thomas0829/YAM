...
def setup_can_interfaces():
    ...
    logger.info("Skipping CAN interface reset...")
...
def _create_robot_client(robot_path_or_robot: Union[str, Robot, List[str]], server_processes: List[Any]) -> Robot:
    ...
    if isinstance(robot_path_or_robot, (str, omegaconf.listconfig.ListConfig, list)):
        ...
        try:
            # Get the base directory (yam_realtime package root)
            import yam_realtime
            base_dir = os.path.dirname(os.path.dirname(yam_realtime.__file__))
            robot_dict = DictLoader.load(robot_path_or_robot, base_dir=base_dir)
