...
class DMSingleMotorCanInterface(CanInterface):
    ...
    def parse_recv_message(
        ...
        # Extract temperature first to check for false over-temperature errors
        temporature_mos = data[6]
        temperature_rotor = data[7]
        
        if error_hex != "0x1":
            # Special handling for over-temperature errors - check actual temperature
            if error_int == 0xC:  # motor_over_temperature
                if temperature_rotor < 80:  # If actual temp is reasonable, it's likely a false alarm
                    logging.warning(
                        f"motor id: {motor_id_of_this_response}, over-temp error but actual temp is {temperature_rotor}°C (likely false alarm) at {self.name} and channel {self.bus.channel_info}"
                    )
                    error_int = 0x1  # Treat as normal
                    error_hex = "0x1"
...
class DMChainCanInterface(MotorChain):
    def __init__(
        ...
        # Give the CAN listener time to fully initialize before attempting motor communication
        time.sleep(0.2)
        self._motor_on()
        starting_command = []
    ...
    def _motor_on(self) -> None:
        ...
        for motor_id, motor_type in self.motor_list:
            ...
            time.sleep(0.05)  # increased from 0.01 to 0.05 for better reliability
